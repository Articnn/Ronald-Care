import { Building2, ClipboardList, HeartHandshake, LayoutDashboard, MenuSquare, Package, Route, Users } from 'lucide-react'
...
const navByRole: Record<Exclude<Role, null>, Array<{ label: string; to: string; icon: ReactElement }>> = {
  ...
  staff: [
    ...
    { label: 'Staff Assist', to: '/staff/family-status', icon: <ClipboardList className="h-4 w-4" /> },
  ...
  family: [
    { label: 'Ayuda Asistida', to: '/kiosk', icon: <MenuSquare className="h-4 w-4" /> },
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-4">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-warm-100">RonaldCare · no clinico</p>
            </div>
          </Link>
          <span className="rounded-full bg-white/15 px-3 py-1 text-sm font-bold">Sede: {site}</span>
          {role ? <span className="rounded-full bg-gold-300 px-3 py-1 text-sm font-bold text-warm-900">Rol: {roleLabels[role]}</span> : null}
